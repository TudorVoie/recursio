...
let array = Array.from({ length: rows2 }, () =>
    Array(cols2).fill(0)
);

let anime = Array.from({ length: rows2 }, () =>
    Array(cols2).fill(0)
);
let drawing_x = Array.from({ length: rows2 }, () =>
    Array(cols2).fill(0)
);
let drawing_y = Array.from({ length: rows2 }, () =>
    Array(cols2).fill(0)
);
...
for (let i = 0; i < rows2; i++) {
    for (let j = 0; j < cols2; j++) {
        if (matrix2[i][j] != -1 && matrix2[i][j] != 0) {
            ...
            drawing_x[i][j] = Math.floor(rect.left + rect.width / 2);
            drawing_y[i][j] = Math.floor(rect.top + rect.height / 2);
        }
    }
}
for (let i = 0; i < rows2; i++)
